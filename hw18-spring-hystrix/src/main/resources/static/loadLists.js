...
function loadBookComments(comments) {
    let content = "";
                    ...
                    function (comments) {
                        ...
                        comments.forEach(function (comment) {
                            content = content.concat('<div class="listElem">',
                                ...
                                '<th>ID:</th>');
                            if (comment.id === 0)
                                content = content.concat('<td>' + comment.id + '</td>');
                            else
                                content = content.concat('<td><a class="contextHref" href="/comment?id=' + comment.id + '">' + comment.id + '</a></td>');
                            ...
                            if (comment.commentDate == null)
                                content = content.concat('<td>N/A</td>');
                            else
                                content = content.concat('<td>' + comment.commentDate + '</td>');
                            content = content.concat('</tr>',
                                '<tr>',
                                '<th>Книга:</th>');
                            if (comment.book.id===0)
                            content = content.concat('<td>N/A</td>');
                            else
                                content = content.concat('<td><a class="contextHref" href="/book?id=' + comment.book.id + '">',
                                    comment.book.author.shortName + ' "' + comment.book.title + '"</a></td>');
                            content = content.concat('</tr>',
                                '<tr>',
                                '<th style="vertical-align: center;">Комментарий:</th>',
                                '<td class="commentText">' + comment.content + '</td>',
                    ...
                    function (books) {
                        ...
                        books.forEach(function (book) {
                            content = content.concat('<div class="listElem">',
                                ...
                                '<th>ID:</th>');
                            if (book.author.id === 0) {
                                content = content.concat('<td>' + book.id + '</td>');
                            } else {
                                content = content.concat('<td><a class="contextHref" href="/book?id=' + book.id + '">' + book.id + '</a></td>');
                            }
                            content = content.concat('</tr><tr><th>Автор:</th>');
                            if (book.author.id === 0) {
                                content = content.concat('<td>N/A</td>');
                            } else {
                                content = content.concat('<td><a class="contextHref" href="/author?id=' + book.author.id + '">' + book.author.shortName + '</a></td>');
                            }
                            content = content.concat('</tr><tr><th>Название:</th>');
                            if (book.author.id === 0)
                                content = content.concat('<td>' + book.title + '</td>');
                            else
                                content = content.concat('<td><a class="contextHref" href="/book?id=' + book.id + '">"' + book.title + '"</a></td>');
                            content = content.concat('</tr><tr><th>Жанр(ы):</th>');
                            if (book.author.id === 0)
                                content = content.concat('<td>N/A</td></tr></table></div>');
                            else
                                content = content.concat('<td>' + getGenresList(book.genres) + '</td></tr></table></div>');
...
function getGenresList(genres) {
    ...
    for (i = 0; i < genres.length; i++) {
        if (genres[i].name !== "N/A")
            result = result.concat('<span style="margin: 0; padding: 0"><a class="contextHref" href="genre?id=' + genres[i].id + '">');
        else
