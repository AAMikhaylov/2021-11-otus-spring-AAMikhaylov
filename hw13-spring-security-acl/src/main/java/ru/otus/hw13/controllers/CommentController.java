...
import ru.otus.hw13.exceptions.BadRequestException;
import ru.otus.hw13.exceptions.IntegrityViolationException;
import ru.otus.hw13.exceptions.NotFoundException;
import ru.otus.hw13.services.BookService;
import ru.otus.hw13.services.CommentService;
...
public class CommentController {
    ...
    public String save(@Valid @ModelAttribute("comment") CommentDto comment, BindingResult bindingResult) {
        ...
        val auth=secContext.getAuthentication();

        val commentToSave=new CommentDto(comment.getId(),auth.getName(),null, comment.getBook(), comment.getContent());
        val savedComment = commentService.save(commentToSave);
